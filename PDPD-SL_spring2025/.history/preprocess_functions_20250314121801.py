...
def create_parcel_arcs_weighted(
    pickup_list,
    delivery_list,
    bus_stop_list,
    dist_drone_arcs,
    ...
    # A simple function to estimate bus wait time 
    # (placeholder, adapt if you have a real schedule)
    def estimate_wait_time():
        # e.g. assume a fixed 5 min average wait
        return 5.0

    for idx, (p, d) in enumerate(zip(pickup_list, delivery_list)):
        # --- Step 1: Direct flight cost
        direct_dist = dist_drone_arcs[p, d]
        # no wait, no bus => cost
        cost_direct = (weights.get('drone_dist', 1.0) * direct_dist)

        # --- Step 2: find "max_top_stations" nearest bus stops for pickup
        # bus_stop_list is something like: bus_stop_list[s][i] => node
        # we want all s for the current parcel i
        stops_for_this_parcel = [one_line[idx] for one_line in bus_stop_list]
        # compute distance p->each stop
        dist_list_p = []
        ...
        # We'll store best bus-based cost and the best (p_stop, d_stop)
        best_bus_cost = float('inf')
        best_bus_pair = None
        # also want to store which arcs we create (p->p_stop, T->T..., d_stop->d)
        best_bus_arcs = []

        # --- Step 3: Try all combos in the top sets
        for (p_stop_node, p_stop_dist) in top_p_stations:
            for (d_stop_node, d_stop_dist) in top_d_stations:
                # 3a) drone portion = p->p_stop + d_stop->d
                # total drone distance
                total_drone_dist = p_stop_dist + d_stop_dist

                # 3b) bus portion from p_stop_node to d_stop_node
                # simple approach: direct T->T from time_bus_arcs
                # (maybe you want to chain multiple T->T if there's multiple stops in between.)
                bus_travel_time = time_bus_arcs[p_stop_node, d_stop_node]

                # 3c) wait time for bus
                # in reality, depends on arrival time at p_stop_node etc.
                wait_time = estimate_wait_time()

                # 3d) Weighted cost
                cost_combo = (
                    weights.get('drone_dist', 1.0)*total_drone_dist
                ...
                if cost_combo < best_bus_cost:
                    ...
                    best_bus_pair = (p_stop_node, d_stop_node)
                    # We'll also remember which arcs we might add 
                    #  for the bus portion as well
                    # ( p->p_stop_node, p_stop_node->d_stop_node, d_stop_node->d )
                    # If you have a chain of T->T arcs, you'd store them all here:
                    best_bus_arcs = [
                        (p, p_stop_node), 
                        (p_stop_node, d_stop_node),
                        (d_stop_node, d)
                    ]

        # --- Step 4: Compare with direct flight
        if cost_direct <= best_bus_cost:
            # direct flight is better (or tie)
            parcel_arcs.append((p, d))
        else:
            # bus-based approach
            # here we do "Add arcs" = p->p_stop, T->T..., d_stop->d
            # from best_bus_arcs
